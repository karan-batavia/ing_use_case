...
class DOCXToTextConverter:
    def __init__(self, include_headers=True, include_footers=True, include_tables=True, 
                 preserve_formatting=False, clean_whitespace=True, remove_empty_lines=True,
                 table_separator=" | ", include_metadata=False):
        """
...
def interactive_mode():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("\n--- Processing Options ---")
            include_headers = input("Include headers? (y/n, default=y): ").strip().lower() in ['', 'y', 'yes']
            include_footers = input("Include footers? (y/n, default=y): ").strip().lower() in ['', 'y', 'yes']
            include_tables = input("Include tables? (y/n, default=y): ").strip().lower() in ['', 'y', 'yes']
            preserve_formatting = input("Preserve formatting (headings, quotes)? (y/n, default=n): ").strip().lower() in ['y', 'yes']
            include_metadata = input("Include document metadata? (y/n, default=n): ").strip().lower() in ['y', 'yes']
            ...
            include_footers = input("Include footers? (y/n, default=y): ").strip().lower() in ['', 'y', 'yes']
            include_tables = input("Include tables? (y/n, default=y): ").strip().lower() in ['', 'y', 'yes']
            preserve_formatting = input("Preserve formatting? (y/n, default=n): ").strip().lower() in ['y', 'yes']
            include_metadata = input("Include metadata? (y/n, default=n): ").strip().lower() in ['y', 'yes']
            clean_ws = input("Clean whitespace? (y/n, default=y): ").strip().lower() in ['', 'y', 'yes']
            remove_empty = input("Remove empty lines? (y/n, default=y): ").strip().lower() in ['', 'y', 'yes']
...
Examples:
  ...
  python docx_to_txt.py file.docx --preserve-formatting --include-metadata
    ...
    # Content options
    parser.add_argument('--no-headers', action='store_true', help='Exclude headers from output')
    parser.add_argument('--no-footers', action='store_true', help='Exclude footers from output')
    parser.add_argument('--no-tables', action='store_true', help='Exclude tables from output')
    parser.add_argument('--preserve-formatting', action='store_true', help='Preserve text formatting (headings, quotes)')
    ...
    parser.add_argument('--table-separator', default=' | ', help='Separator for table cells (default: " | ")')
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        converter = DOCXToTextConverter()
        input_docx = "example.docx"  # Replace with your DOCX file path
