...
def _generate_detection_explanation(
    detection_type: str, detection_method: str, confidence: float
) -> Dict[str, Any]:
    """Generate explanation and metadata for a detection"""

    # Entity type explanations
    explanations = {
        "EMAIL": "email address pattern",
        "PHONE_EU": "European phone number format",
        ...
        "BIOMETRIC": "biometric data reference",
    ...
    method_explanations = {
        ...
        "hybrid": "combined NER and pattern matching",
    }
    ...
    entity_desc = explanations.get(detection_type, f"{detection_type} pattern")
    method_desc = method_explanations.get(detection_method, detection_method)

    explanation = f"Detected as {entity_desc} using {method_desc}"
    if confidence < 1.0:
    ...
    return {
        ...
        "can_override": confidence
        < 0.95,  # High-confidence detections harder to override
        "risk_level": risk_levels.get(detection_type, "medium"),
    }
        ...
        detections.append(
            {
                ...
                "method": "regex",  # Custom patterns use regex
...
class RedactRequest(BaseModel):
    ...
    def validate_method(cls, v):
        if v not in ["auto", "enhanced", "legacy"]:
            raise ValueError("method must be one of: auto, enhanced, legacy")
        return v
...
class Detection(BaseModel):
    ...
    explanation: str  # e.g., "detected as IBAN using regex pattern"
    confidence: float  # 0.0 to 1.0
    detection_method: str  # "regex", "ner", "hybrid"
    can_override: bool = True  # Whether this detection can be overridden
    risk_level: str = "medium"  # "low", "medium", "high", "critical"


class OverrideRequest(BaseModel):
    """Request to override specific detections"""

    session_id: str
    detection_overrides: List[
        Dict[str, Any]
    ]  # List of detection indices and justifications
    justification: str
    override_reason: str  # "false_positive", "business_requirement", "legal_compliance"


class OverrideResponse(BaseModel):
    """Response for detection override"""

    success: bool
    session_id: str
    original_text: str
    revised_redacted_text: str
...
@app.get("/redact/info")
async def get_redaction_info():
    """
    Get information about available redaction methods and sensitivity levels.
    No authentication required for informational endpoint.
    """
    return {
        "methods": {
            "enhanced": "Hybrid NER+regex model with advanced entity detection (recommended)",
            "legacy": "Original regex-only model with sensitivity level filtering",
        },
        "sensitivity_levels": {
            "C1": "Public - Minimal redaction (names, phone numbers only)",
            "C2": "Internal - Standard redaction (emails, phones, IDs)",
            "C3": "Restricted - High redaction (financial data, personal info)",
            "C4": "Confidential - Maximum redaction (all sensitive data)",
        },
        "enhanced_model_available": enhanced_redaction_model is not None,
        "status": (
            "Enhanced model loaded"
            if enhanced_redaction_model
            else "Using legacy model only"
        ),
        "default_method": "enhanced" if enhanced_redaction_model else "legacy",
        "fallback_behavior": "Automatically falls back to legacy if enhanced model fails",
    }
...
async def redact_sensitive_data(
    ...
    try:
        ...
        # Determine which method to use based on request and availability
        use_enhanced = False

        if request.method == "enhanced":
            if enhanced_redaction_model is None:
                raise HTTPException(
                    status_code=503,
                    detail="Enhanced redaction model is not available. Use method='auto' or 'legacy'.",
                )
            use_enhanced = True
        elif request.method == "auto":
            use_enhanced = enhanced_redaction_model is not None
        elif request.method == "legacy":
            use_enhanced = False
        ...
        if use_enhanced:
            ...
            try:
                ...
                for entity_detection in enhanced_result.detections:
                    ...
                    detection = {
                        ...
                        "placeholder": placeholder,
                        "method": "hybrid",  # Enhanced model uses hybrid approach
                        "confidence": entity_detection.confidence,  # Use actual confidence from model
                    }
                    detections.append(detection)
                ...
                result_text = enhanced_result.redacted_text
                total_redacted = len(detections)
                method_used = "enhanced"
                confidence_scores = {"overall": enhanced_result.confidence}
                entities_found = [detection["type"] for detection in detections]

            except Exception as e:
                # Fallback to legacy if enhanced model fails
                print(f"Enhanced model failed, falling back to legacy: {e}")
            ...
            # Enhanced model not available, use legacy with sensitivity level filtering
            method_used = "legacy"
            confidence_scores = None
            entities_found = None
        ...
        # Convert detections to Pydantic models with explainability
        detection_models = []
        for d in detections:
            # Generate explanation metadata
            confidence = float(d.get("confidence", 0.9))  # Ensure float type
            explanation_data = _generate_detection_explanation(
                d["type"],
                d.get("method", "regex"),  # Default to regex for legacy detections
                confidence,
            )
            ...
            detection_models.append(
                Detection(
                    ...
                    detection_method=explanation_data["detection_method"],
                    can_override=explanation_data["can_override"],
...
@app.post("/redact/override", response_model=OverrideResponse)
async def override_redaction_detections(
    request: OverrideRequest,
    current_user: Dict[str, Any] = Depends(get_current_user),
    audit_context: AuditContext = Depends(get_request_context),
):
    """
    Override specific redaction detections with justification.
    Allows users to mark detections as false positives or provide business justification.
    Requires user authentication and logs all override actions.
    """
    try:
        # Get the original redaction record
        ...
        original_detections = details.get("detections", [])

        if not original_detections:
            raise HTTPException(
                status_code=400, detail="No detections found for override"
            )

        # Apply overrides
        revised_redacted_text = original_text
        overrides_applied = 0

        # Process each override request
        for override_item in request.detection_overrides:
            detection_index = override_item.get("detection_index")
            should_keep_original = override_item.get("keep_original", False)

            if detection_index is not None and detection_index < len(
                original_detections
            ):
                detection = original_detections[detection_index]

                if should_keep_original:
                    # Don't redact this detection - keep original text
                    placeholder = detection.get("placeholder", "")
                    original_value = detection.get("original", "")

                    if placeholder and original_value:
                        revised_redacted_text = revised_redacted_text.replace(
                            placeholder, original_value
                        )
                        overrides_applied += 1

        # Enhanced audit logging for override action (critical operation)
        enhanced_audit_log(
            session_id=request.session_id,
            user_id=current_user["user_id"],
            action="redaction_override",
            details={
                "original_session_id": request.session_id,
                "justification": request.justification,
                "override_reason": request.override_reason,
                "overrides_requested": len(request.detection_overrides),
                "overrides_applied": overrides_applied,
                "original_text": original_text,
                "revised_redacted_text": revised_redacted_text,
                "critical_operation": True,
            },
            audit_context=audit_context,
            endpoint="/redact/override",
        )
        ...
        return OverrideResponse(
            ...
            revised_redacted_text=revised_redacted_text,
            overrides_applied=overrides_applied,
            message=f"Successfully applied {overrides_applied} detection overrides",
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error processing override request: {str(e)}"
        )
