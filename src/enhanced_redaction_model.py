...
class EnhancedBankingNER:
    ...
    def _load_spacy_model(self):
        ...
        if self.nlp is None:
            # Create a blank model if no pre-trained model available
            try:
                self.nlp = spacy.blank("en")
                logger.warning("Using blank spaCy model - NER features limited")
            except Exception:
                raise RuntimeError(
                    "Cannot initialize spaCy model. Install with: python -m spacy download en_core_web_sm"
                )
    ...
    def _add_banking_patterns(self):
        ...
        try:
            if "entity_ruler" not in self.nlp.pipe_names:
                entity_ruler = self.nlp.add_pipe(
                    "entity_ruler",
                    before="ner" if "ner" in self.nlp.pipe_names else "last",
                )
            else:
    ...
    def extract_entities(self, text: str) -> List[EntityDetection]:
        ...
        try:
            ...
            for ent in doc.ents:
                # Map spaCy labels to our banking labels
    ...
    def _calculate_confidence(self, ent) -> float:
        ...
        # For custom patterns, confidence is high
        ...
        # For standard NER, use a heuristic based on entity type and context
        confidence_map = {
            "PERSON": 0.85,
            "ORG": 0.80,
            "MONEY": 0.90,
...
class SensitivityClassifier:
    ...
    def predict(self, text: str) -> Tuple[str, float]:
        ...
        if not hasattr(self, "pipeline") or self.pipeline is None:
            return "C2", 0.5  # Default fallback
...
class EnhancedRedactionModel:
    ...
    def _merge_detections(
        ...
        for next_detection in detections[1:]:
            ...
            if next_detection.start <= current.end:
                ...
                # If same confidence, prefer NER over regex
                elif (
    ...
    def _filter_by_sensitivity(
        ...
        if sensitivity_level == "C1":
            return []  # No redaction for public info

        if sensitivity_level not in self.sensitivity_rules:
            return detections  # Full redaction if unknown level

        allowed_types = self.sensitivity_rules[sensitivity_level]
        if allowed_types is None:
            return detections  # Full redaction for C4
